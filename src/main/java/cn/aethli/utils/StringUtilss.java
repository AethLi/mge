...
public class StringUtilss {
	...
	public static boolean hasBlank(String... strs) {
		if(CollectionUtilss.isEmpty(strs)) {
			return true;
		}
	...
	public static String format(String template, Object... values) {
		if (CollectionUtilss.isEmpty(values) || isBlank(template)) {
			return template;
		}
	...
	/**
	 * 将list集合进行遍历，形成String字符串，以分隔符进行分割
	 * @param list	list集合
	 * @param separatorStr	分隔符
	 * @return
	 */
	public static String getStringSeparatorByList(List<?> list, String separatorStr) {
		StringBuffer sBuffer = new StringBuffer();
		for(Object _obj : list) {
			sBuffer.append(_obj + separatorStr);
		}
