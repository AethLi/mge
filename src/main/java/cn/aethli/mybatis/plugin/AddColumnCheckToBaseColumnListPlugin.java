package cn.aethli.mybatis.plugin;
 ...
 * 	为了考虑数据权限的列权限部分的功能，则需要在查询出来的所有列的部分添加权限过滤功能，如下所示，生成的	<sql id="Base_Column_List"> 部分，即增加了 myColumn_ 
 *  开头的权限控制部分的功能；
 * 
 * <sql id="Base_Column_List">
		student.ID as student_ID,
		<if test="myColumn_name == null or myColumn_name == ''">
	...
	public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
		...
		for (IntrospectedColumn introspectedColumn : introspectedTable
			...
			valuesNotNullElement.addAttribute(new Attribute(
			        "test", strIf.toString())); //$NON-NLS-1$

			strIf.setLength(0);
			strIf.append(MyBatis3FormattingUtilities
